...
- Post-completion correction: wrapped completion notifications in Pi's standard `customMessageBg` `Box` and applied `customMessageLabel`, `customMessageText`, accent, and semantic state colors instead of rendering unstyled `Text`.
